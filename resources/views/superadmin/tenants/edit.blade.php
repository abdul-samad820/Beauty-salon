...
@section('topbar-actions')
  <a href="{{ route('superadmin.tenants.show', $tenant) }}" class="btn-ghost">
...
<style>
  ...
  .fl-group label { position:absolute; top:0; left:1rem; font-size:0.65rem; font-weight:600; letter-spacing:0.12em; color:var(--gold); background:var(--bg-card); padding:0 0.4rem; transform:translateY(-50%); }
  .fl-group.has-icon label { left:2.8rem; }
  .fl-group input, .fl-group select, .fl-group textarea { width:100%; background:var(--bg-input); border:1px solid var(--border-2); border-radius:10px; color:var(--text); font-family:var(--ff-body); font-size:0.85rem; font-weight:300; padding:0.9rem 1rem; outline:none; transition:border-color 0.3s; appearance:none; }
  .fl-group.has-icon input, .fl-group.has-icon select { padding-left:2.8rem; }
  .fl-group textarea { padding-top:1.2rem; resize:vertical; min-height:80px; }
  ...
  .danger-zone { background:var(--rose-dim); border:1px solid rgba(244,63,94,0.2); border-radius:12px; padding:1.5rem; }
</style>
@endpush
  ...
  <div class="col-xl-8 col-lg-10">
    <form method="POST" action="{{ route('superadmin.tenants.update', $tenant) }}">
      ...
      <div class="form-card fade-in-up">
        ...
        <div class="form-card-sub">Tenant ki core details update karein</div>
        <div class="row g-3">
          <div class="col-md-6">
      ...
      <div class="form-card fade-in-up stagger-2">
        ...
        <div class="form-card-sub">Tenant ka status change karein</div>
        <div class="fl-group">
          <select name="status_note" style="width:100%;background:var(--bg-input);border:1px solid var(--border-2);border-radius:10px;color:var(--text);font-family:var(--ff-body);font-size:0.85rem;padding:0.9rem 1rem;outline:none;appearance:none;">
        ...
        <div style="font-size:0.72rem;color:var(--text-3);">
          <i class="bi bi-info-circle"></i> Suspended karne par tenant ke saare users ka access band ho jaayega.
        </div>
      </div>
    ...
    <div class="danger-zone fade-in-up stagger-3">
      ...
      <div style="font-size:0.78rem;color:var(--text-2);margin-bottom:1rem;">
        Tenant delete karne par uske saare data ka access band ho jaayega. Ye action reversible nahi hai.
      </div>
      <form method="POST" action="{{ route('superadmin.tenants.destroy', $tenant) }}"
        onsubmit="return confirm('Aap sure hain? Tenant {{ addslashes($tenant->name) }} suspend ho jaayega.')">
        @csrf @method('DELETE')
