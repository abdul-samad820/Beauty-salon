...
@push('styles')
<style>
  .stepper-wrap { display:flex; align-items:flex-start; gap:0; margin-bottom:2rem; position:relative; }
  .stepper-wrap::before { content:''; position:absolute; top:18px; left:0; right:0; height:1px; background:var(--border-2); z-index:0; }
  .step { display:flex; flex-direction:column; align-items:center; gap:0.5rem; flex:1; position:relative; z-index:1; }
  .step-circle { width:36px; height:36px; border-radius:50%; border:1.5px solid var(--border-2); background:var(--bg-card); display:flex; align-items:center; justify-content:center; font-size:0.78rem; font-weight:500; color:var(--text-3); transition:all 0.4s; }
  ...
  .step-label { font-size:0.62rem; font-weight:600; letter-spacing:0.15em; text-transform:uppercase; color:var(--text-3); text-align:center; transition:color 0.4s; white-space:nowrap; }
  .step.active .step-label { color:var(--gold); }
  .step.done .step-label   { color:var(--emerald); }
  .form-panel { display:none; animation:panelIn 0.5s ease both; }
  ...
  .form-card::before { content:''; position:absolute; top:0; left:0; right:0; height:1px; background:linear-gradient(90deg,transparent,rgba(255,255,255,0.08),transparent); }
  .form-card-title { font-family:var(--ff-display); font-size:1.2rem; font-weight:400; color:var(--text); margin-bottom:0.3rem; }
  .form-card-sub   { font-size:0.75rem; color:var(--text-3); margin-bottom:1.8rem; }
  /* Floating labels */
  .fl-group { position:relative; margin-bottom:1.4rem; }
  .fl-group label { position:absolute; top:50%; left:1rem; transform:translateY(-50%); font-size:0.82rem; color:var(--text-3); pointer-events:none; transition:all 0.25s; background:transparent; padding:0 0.2rem; letter-spacing:0.04em; }
  .fl-group.has-icon label { left:2.8rem; }
  .fl-group input, .fl-group select, .fl-group textarea { width:100%; background:var(--bg-input); border:1px solid var(--border-2); border-radius:10px; color:var(--text); font-family:var(--ff-body); font-size:0.85rem; font-weight:300; padding:0.9rem 1rem; outline:none; transition:border-color 0.3s, background 0.3s, box-shadow 0.3s; appearance:none; }
  ...
  .fl-group input:focus, .fl-group select:focus, .fl-group textarea:focus { border-color:var(--gold); background:rgba(201,169,110,0.04); box-shadow:0 0 0 3px rgba(201,169,110,0.08); }
  ...
  .fl-group textarea:not(:placeholder-shown) + label { top:0; font-size:0.65rem; font-weight:600; letter-spacing:0.12em; color:var(--gold); background:var(--bg-card); padding:0 0.4rem; }
  ...
  .field-hint { font-size:0.68rem; color:var(--text-3); margin-top:0.3rem; }
  /* Plan cards */
  .plan-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(200px,1fr)); gap:1rem; margin-bottom:1.5rem; }
  .plan-card { background:rgba(255,255,255,0.03); border:1.5px solid var(--border-2); border-radius:12px; padding:1.5rem; cursor:pointer; transition:all 0.35s; position:relative; overflow:hidden; }
  .plan-card:hover { border-color:var(--gold); transform:translateY(-2px); }
  .plan-card.selected { border-color:var(--gold); background:var(--gold-dim); box-shadow:0 8px 32px rgba(201,169,110,0.2); }
  .plan-check { position:absolute; top:0.8rem; right:0.8rem; width:22px; height:22px; border-radius:50%; border:1.5px solid var(--border-2); display:flex; align-items:center; justify-content:center; transition:all 0.3s; font-size:0.7rem; color:transparent; }
  ...
  .plan-name  { font-family:var(--ff-display); font-size:1.3rem; font-weight:400; color:var(--text); margin-bottom:0.2rem; }
  .plan-price { font-family:var(--ff-display); font-size:1.6rem; font-weight:400; margin-bottom:0.8rem; }
  .plan-price small { font-family:var(--ff-body); font-size:0.7rem; color:var(--text-3); }
  .plan-features { list-style:none; display:flex; flex-direction:column; gap:0.4rem; }
  .plan-features li { font-size:0.75rem; color:var(--text-2); display:flex; align-items:center; gap:0.5rem; }
  .plan-features li i { color:var(--emerald); font-size:0.75rem; }
  /* Review block */
  .review-block { background:rgba(255,255,255,0.02); border:1px solid var(--border); border-radius:10px; padding:1.2rem; margin-bottom:1rem; }
  .review-label { font-size:0.6rem; font-weight:600; letter-spacing:0.2em; text-transform:uppercase; color:var(--text-3); margin-bottom:0.8rem; }
  .review-row { display:flex; justify-content:space-between; padding:0.4rem 0; border-bottom:1px solid rgba(255,255,255,0.03); font-size:0.82rem; }
  ...
  .form-progress-fill { height:100%; background:linear-gradient(90deg,var(--teal),var(--gold)); border-radius:2px; transition:width 0.6s cubic-bezier(0.22,1,0.36,1); }
  @media(max-width:576px) { .plan-grid { grid-template-columns:1fr; } .stepper-wrap { gap:0.5rem; } }
</style>
@endpush
...
<div class="row justify-content-center">
  <div class="col-xl-9 col-lg-11">
    ...
    <div class="stepper-wrap fade-in-up stagger-1">
      <div class="step active" id="step-dot-1"><div class="step-circle" id="sc1">1</div><div class="step-label">Business Info</div></div>
      <div class="step" id="step-dot-2"><div class="step-circle" id="sc2">2</div><div class="step-label">Choose Plan</div></div>
      <div class="step" id="step-dot-3"><div class="step-circle" id="sc3">3</div><div class="step-label">Settings</div></div>
      <div class="step" id="step-dot-4"><div class="step-circle" id="sc4">4</div><div class="step-label">Review & Launch</div></div>
    </div>
    ...
    <form method="POST" action="{{ route('superadmin.tenants.store') }}" id="mainForm">
      ...
      <div class="form-panel active" id="panel-1">
        <div class="form-card fade-in-up stagger-2">
          ...
          <div class="form-card-sub">Naye salon tenant account ki core details bharein</div>
          <div class="row g-3">
            <div class="col-md-6">
        ...
        <div style="display:flex;justify-content:flex-end;">
          <button type="button" class="btn-gold" onclick="goTo(2)">Next: Choose Plan <i class="bi bi-arrow-right"></i></button>
        </div>
      </div>
      ...
      <div class="form-panel" id="panel-2">
        <div class="form-card fade-in-up stagger-1">
          ...
          <div class="form-card-sub">Tenant ke liye plan choose karein</div>
          <input type="hidden" name="plan" id="planInput" value="{{ old('plan', 'pro') }}" />
          <div class="plan-grid">
            {{-- Free --}}
            <div class="plan-card {{ old('plan', 'pro') === 'free' ? 'selected' : '' }}" onclick="selectPlan(this,'free')">
              ...
              <ul class="plan-features">
                <li><i class="bi bi-check-circle-fill"></i> 1 Staff</li>
                <li><i class="bi bi-check-circle-fill"></i> 10 Services</li>
                <li><i class="bi bi-check-circle-fill"></i> Basic Reports</li>
              </ul>
            </div>
            {{-- Pro --}}
            <div class="plan-card {{ old('plan', 'pro') === 'pro' ? 'selected' : '' }}" onclick="selectPlan(this,'pro')">
              <div class="plan-check"><i class="bi bi-check"></i></div>
            ...
            {{-- Enterprise --}}
            <div class="plan-card {{ old('plan') === 'enterprise' ? 'selected' : '' }}" onclick="selectPlan(this,'enterprise')">
              <div class="plan-check"><i class="bi bi-check"></i></div>
              <div class="plan-icon" style="background:var(--gold-dim);color:var(--gold);"><i class="bi bi-crown-fill"></i></div>
              <div class="plan-name">Enterprise</div>
              <div class="plan-price" style="color:var(--gold);">Custom <small>pricing</small></div>
              <ul class="plan-features">
                ...
                <li><i class="bi bi-check-circle-fill"></i> Custom Subdomain</li>
              </ul>
            </div>
          </div>
        </div>
        <div style="display:flex;justify-content:space-between;">
          <button type="button" class="btn-ghost" onclick="goTo(1)"><i class="bi bi-arrow-left"></i> Back</button>
          <button type="button" class="btn-gold" onclick="goTo(3)">Next: Settings <i class="bi bi-arrow-right"></i></button>
        </div>
      </div>
      ...
      <div class="form-panel" id="panel-3">
        <div class="form-card fade-in-up stagger-1">
          ...
          <div class="form-card-sub">Ye settings baad mein owner bhi change kar sakta hai</div>
          <div class="row g-3">
            <div class="col-md-6">
              <div class="fl-group has-icon">
        ...
        <div style="display:flex;justify-content:space-between;">
          <button type="button" class="btn-ghost" onclick="goTo(2)"><i class="bi bi-arrow-left"></i> Back</button>
          <button type="button" class="btn-gold" onclick="goTo(4)">Review & Launch <i class="bi bi-arrow-right"></i></button>
        </div>
      </div>
      ...
      <div class="form-panel" id="panel-4">
        <div class="form-card fade-in-up stagger-1">
          ...
          <div class="form-card-sub">Confirm karo aur tenant create karo</div>
          ...
          <div style="background:var(--emerald-dim);border:1px solid rgba(16,185,129,0.2);border-radius:8px;padding:1rem;margin-top:0.5rem;">
            <div style="font-size:0.78rem;color:var(--emerald);font-weight:500;">
              <i class="bi bi-check-circle-fill"></i> Owner ke liye login credentials automatically set ho jaayenge.
            </div>
          </div>
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <button type="button" class="btn-ghost" onclick="goTo(3)"><i class="bi bi-arrow-left"></i> Back</button>
          <button type="submit" class="btn-gold" style="padding:0.7rem 2rem;font-size:0.8rem;">
...
let currentStep = 1;
const totalSteps = 4;
const progressPcts = {1:25, 2:50, 3:75, 4:100};
