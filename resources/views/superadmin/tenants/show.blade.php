...
@section('topbar-actions')
  <a href="{{ route('superadmin.tenants.edit', $tenant) }}" class="btn-ghost">
  ...
  @if($tenant->status !== 'active')
    <form method="POST" action="{{ route('superadmin.tenants.status', $tenant) }}" style="display:inline;">
      ...
      <button type="submit" class="btn-ghost" style="border-color:var(--emerald);color:var(--emerald);">
  ...
  @else
    <form method="POST" action="{{ route('superadmin.tenants.status', $tenant) }}" style="display:inline;">
      ...
      <button type="submit" class="btn-danger" onclick="return confirm('Suspend karna chahte hain?')">
...
@push('styles')
<style>
  .info-row { display:flex; justify-content:space-between; padding:0.55rem 0; border-bottom:1px solid rgba(255,255,255,0.03); font-size:0.82rem; }
  .info-row:last-child { border-bottom:none; }
  .info-key { color:var(--text-3); }
  .info-val { color:var(--text); font-weight:400; text-align:right; }
  .detail-tabs { display:flex; gap:0; border-bottom:1px solid var(--border); margin-bottom:1.5rem; overflow-x:auto; }
  .detail-tab { padding:0.65rem 1.2rem; font-size:0.78rem; color:var(--text-3); cursor:pointer; border-bottom:2px solid transparent; transition:color 0.25s, border-color 0.25s; white-space:nowrap; }
  .detail-tab.active { color:var(--gold); border-bottom-color:var(--gold); }
  .detail-tab:hover:not(.active) { color:var(--text-2); }
  .tab-panel { display:none; }
  .tab-panel.active { display:block; }
  .stat-mini { background:var(--bg-card-2); border:1px solid var(--border); border-radius:10px; padding:1rem; }
  .stat-mini-val { font-family:var(--ff-display); font-size:1.8rem; font-weight:400; line-height:1; margin-bottom:0.3rem; }
  .stat-mini-label { font-size:0.65rem; color:var(--text-3); letter-spacing:0.12em; text-transform:uppercase; }
  .staff-table { width:100%; border-collapse:collapse; }
  .staff-table th { font-size:0.6rem; font-weight:600; letter-spacing:0.2em; text-transform:uppercase; color:var(--text-3); padding:0.7rem 1rem; border-bottom:1px solid var(--border); text-align:left; }
  .staff-table td { padding:0.8rem 1rem; font-size:0.8rem; color:var(--text-2); border-bottom:1px solid rgba(255,255,255,0.03); }
...
@section('content')
  ...
  {{-- ── HEADER CARD ── --}}
  <div class="card-glass p-4 mb-3 fade-in-up">
  ...
  <div class="tab-panel active" id="tab-overview">
    <div class="row g-3">
      ...
      <div class="col-6 col-md-3">
        <div class="stat-mini"><div class="stat-mini-val" style="color:var(--emerald);">{{ $stats['completed'] }}</div><div class="stat-mini-label">Completed</div></div>
      </div>
      <div class="col-6 col-md-3">
        <div class="stat-mini"><div class="stat-mini-val" style="color:var(--rose);">{{ $stats['cancelled'] }}</div><div class="stat-mini-label">Cancelled</div></div>
      </div>
      <div class="col-6 col-md-3">
        <div class="stat-mini"><div class="stat-mini-val" style="color:var(--gold);">{{ $stats['staff_count'] }}</div><div class="stat-mini-label">Staff</div></div>
      </div>
      <div class="col-6 col-md-3">
        <div class="stat-mini"><div class="stat-mini-val" style="color:#a78bfa;">{{ $stats['services_count'] }}</div><div class="stat-mini-label">Services</div></div>
      </div>
      ...
      <div class="col-lg-8">
        <div class="card-glass p-4">
          <div class="section-hdr">
            <div>
              <div class="section-hdr-title">Revenue Trend</div>
              <div class="section-hdr-sub">Monthly revenue — last 6 months</div>
            </div>
          </div>
  ...
  <div class="tab-panel" id="tab-analytics">
    <div class="row g-3">
      <div class="col-12">
        <div class="card-glass p-4">
          <div class="section-hdr-title mb-3">Monthly Revenue</div>
    ...
    <div class="card-glass">
      <div style="overflow-x:auto;">
        <table class="staff-table">
          ...
          <tbody>
            @forelse($tenant->users->filter(fn($u) => !$u->hasRole('owner')) as $user)
            <tr>
            ...
            @empty
            <tr>
              <td colspan="5" style="text-align:center;padding:2rem;color:var(--text-3);">Koi staff nahi mili</td>
            </tr>
            @endforelse
          </tbody>
        </table>
  ...
  <div class="tab-panel" id="tab-services">
    <div class="card-glass">
      <div style="overflow-x:auto;">
        <table class="staff-table">
          <thead>
            <tr><th>Service Name</th><th>Category</th><th>Duration</th><th>Price</th><th>Active</th></tr>
          </thead>
          <tbody>
            ...
            @empty
            <tr><td colspan="5" style="text-align:center;padding:2rem;color:var(--text-3);">Koi service nahi mili</td></tr>
            @endforelse
          </tbody>
        </table>
      </div>
  ...
  <div class="tab-panel" id="tab-activity">
    <div class="card-glass p-4">
      <div class="section-hdr-title mb-3">Recent Appointments</div>
      <table class="staff-table">
        <thead>
        ...
        <tbody>
          ...
          @empty
          <tr><td colspan="5" style="text-align:center;padding:2rem;color:var(--text-3);">Koi appointment nahi mili</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
  ...
  if (id === 'tab-analytics' && !window._analyticsInit) {
    ...
    new Chart(document.getElementById('analyticsChart').getContext('2d'), {
      ...
      data: {
        ...
        datasets: [{ label:'Revenue (₹)', data: Object.values(data), backgroundColor:'#c9a96e99', borderColor:'#c9a96e', borderWidth:1, borderRadius:4 }]
      },
...
new Chart(document.getElementById('revenueChart').getContext('2d'), {
  ...
  data: {
    ...
    datasets: [{ label:'₹ Revenue', data: Object.values(revData), borderColor:'#c9a96e', borderWidth:2, fill:true, backgroundColor:'rgba(201,169,110,0.08)', tension:0.4, pointRadius:3, pointBackgroundColor:'#c9a96e' }]
  },
  options: { responsive:true, maintainAspectRatio:false, plugins:{ legend:{display:false} }, scales:{ x:{grid:{display:false}}, y:{grid:{color:'rgba(255,255,255,0.04)'}} } }
});
</script>
