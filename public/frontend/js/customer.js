...
const Booking = {
    ...
    fetchSlots(date, subdomain) {
        ...
        const params = new URLSearchParams({ date, service_id: this.selectedService });
        if (this.selectedStaff) params.append('staff_id', this.selectedStaff);

        fetch(`/${subdomain}/slots?${params.toString()}`, {
    ...
    refreshSubmitState() {
        ...
        const paymentSelected = paymentEl && paymentEl.value !== '';

        const ready = this.selectedService && this.selectedDate && this.selectedSlot && paymentSelected;
        btn.disabled       = !ready;
